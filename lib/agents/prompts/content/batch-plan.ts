...
Aim for 10–18 slots across 14 days.`,
  systemForPlatforms(platforms: ContentPlatform[]) {
    const list = platforms.join("|");
    return `You are Simba AI Content Planning Agent. Propose a 2-week content mix across pillars and platforms.
Balance pillar target percentages. Prefer variety of formats. Do not write full copy yet — topics only.
CRITICAL: Only use these platforms: ${platforms.join(", ")}. Never invent other platforms.
${PLATFORM_RULES}
${CONTENT_JSON_CONTRACT}
Shape:
{
...
Aim for 10–18 slots across 14 days.`;
  ...
  buildUserPrompt(input: {
    ...
    const platformsNote = input.enabledPlatforms?.length
      ? `\n- Enabled platforms only: ${input.enabledPlatforms.join(", ")}`
      : "";
...
- End: ${input.endDate}
- Brief: ${input.brief}
- Pillar targets: ${JSON.stringify(input.pillars)}${platformsNote}
