    ...
    <section className="space-y-3 rounded-xl border p-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Pick from the brand media library (or upload new into the library).
          Instagram needs at least one image before scheduling.
        </p>
      </div>
      {item.platform === "instagram" && media.length === 0 ? (
        <Alert variant="destructive">
          <AlertDescription>
            Attach an image before scheduling this Instagram post.
          </AlertDescription>
        </Alert>
      ) : null}
      {media.length === 0 ? (
      ...
      {canWrite ? (
        <div className="flex flex-wrap gap-2 border-t pt-3">
          <MediaLibraryPicker
            ...
            canWrite={canWrite}
            triggerLabel={media.length ? "Swap from library" : "Choose from library"}
            replace
          />
        </div>
      ) : null}
